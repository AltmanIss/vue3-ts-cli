    ...
    <TableBody>
      ...
      <template #default>
        ...
        >
          ...
          <el-table-column align="center" label="名称" prop="nickName" width="100" />
          ...
          <el-table-column align="center" label="所属部门" prop="departmentName" />
      ...
      <template #content>
        ...
        >
          ...
          <el-form-item class="form-item__require" label="邮箱地址">
            <el-input v-model="userModel.email" size="small" placeholder="请输入邮箱地址" clearable>
            </el-input>
          </el-form-item>
          ...
          <el-form-item class="form-item__require" label="所属角色" prop="path">
            <el-select size="small" placeholder="请选择角色" v-model="userModel.roleId" clearable>
              <el-option
...
function onDeleteItems() {
  if (selectRows.value.length > 0) {
    ElMessageBox.confirm('确定要删除这些用户吗？', '提示')
      .then(() => {
        ElMessage.success(
          '数据模拟删除成功, 参数为：' + JSON.stringify(selectRows.value.map((it: any) => it.id))
        );
      })
      .catch(console.log);
  }
...
function onEnableItem(item: any) {
  ElMessageBox.confirm('确定要' + (item.status === 1 ? '禁用' : '启用') + '此用户吗？', '提示')
    .then(() => {
      ElMessage.success('模拟成功, 参数为：' + JSON.stringify({ uid: item.id }));
    })
    .catch(console.log);
}
