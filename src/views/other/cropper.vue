    ...
    <el-row :gutter="10">
      ...
      <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
        <div style="width: 100%; height: 300px; overflow: hidden; background: #f7f7f7">
          <img style="width: 100%; height: 100%" :src="cropperSrc" />
        </div>
      </el-col>
    </el-row>
  </el-card>
...
<script lang="ts">
import { defineComponent, onMounted, reactive, ref, unref } from '@vue/runtime-core';
  ...
  setup() {
    ...
    onMounted(() => {
      cropper = new Cropper(unref(cropperImg) as HTMLImageElement, {
        crop() {
          cropperSrc.value = cropper?.getCroppedCanvas().toDataURL('image/jpeg', 0.7) || '';
        }
      });
    });
