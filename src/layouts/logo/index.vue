...
export default defineComponent({
  ...
  setup() {
    ...
    const textColor = ref(state.theme === 'light' ? '#001428' : 'var(--el-color-white)');
    watch(
      () => state.theme,
      (newVal) => {
        bgColor.value = newVal === 'light' ? 'var(--el-color-white)' : '#001428';
        textColor.value = state.theme === 'light' ? '#001428' : 'var(--el-color-white)';
      }
    );
    return {
      state,
