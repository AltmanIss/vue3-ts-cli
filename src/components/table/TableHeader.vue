        ...
        <div v-show="collapsedState">
          ...
          >
            <el-col
              ...
              :key="index"
              :span="
                $isMobile ? 24 : row.length === 1 && item.type === 'action' ? 24 : item.span || 8
              "
            >
              ...
              <div v-else-if="item.type === 'action'" class="flex justify-end">
                <el-button type="success" size="small" :icon="RefreshIcon" @click="doResetSearch"
                  >重置</el-button
                >
                <el-button type="primary" size="small" :icon="SearchIcon" @click="doSearch"
                  >搜索</el-button
                >
              </div>
            </el-col>
...
import { Search as SearchIcon, Refresh as RefreshIcon } from '@element-plus/icons';
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const showArrow = computed(() => {
      return props.canCollapsed && !!props.searchModel && props.searchModel.length !== 0;
    });
